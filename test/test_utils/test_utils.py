...
@pytest.fixture
def sample_gdf():
    """Creates a GeoDataFrame with two square polygons and a 'NAME'
    column."""
    data = {
...
def test_point_within_geometry_match(sample_gdf):
    ...
    matching record from the GeoDataFrame."""
    lat, lng = 0.5, 0.5
...
def test_point_within_geometry_none(sample_gdf):
    """Tests that a point outside all polygons returns None when using
    _point_within_geometry."""
    lat, lng = 10.0, 10.0
    result = _point_within_geometry(lat, lng, sample_gdf)
...
def test_point_intersects_geometry_match(sample_gdf):
    """Tests that a point on the boundary of a polygon is not 'within'
    but is detected as an 'intersect'."""
    lat, lng = 0.0, 0.5
...
def test_point_intersects_geometry_none(sample_gdf):
    """Tests that a point completely outside all polygons returns None
    when using _point_intersects_geometry."""
    lat, lng = -5.0, -5.0
    result = _point_intersects_geometry(lat, lng, sample_gdf)
    assert result is None
