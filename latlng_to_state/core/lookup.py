...
from latlng_to_state.utils.utils_core import _point_within_geometry, _point_intersects_geometry
...
def lookup_geometry_by_point(
    ...
    Args:
        ...
        check_type: 'contain' or 'intersect'. If None, try 'contain' then 'intersect'.
        gdf: GeoDataFrame to use for lookup. Must be provided.
        return_col: Column name to return from the matched row.

    Returns:
        The value from the specified column in the matching row, or None if no match.

    Raises:
        ValueError: If `gdf` is not provided or `return_col` is missing in the match.
    ...
    if len(geometry_cols) == 0:
        raise ValueError("Provided GeoDataFrame must contain at least one geometry column.")
    ...
    if match is not None:
        if return_col in match:
            ...
            raise ValueError(f"Column '{return_col}' not found in matched geometry row.")
